...
Ta mission est d'écrire et de maintenir une suite de tests unitaires pour ce projet, **jusqu'à atteindre une couverture de code ≥ 90 %**.

## Contexte technique

- **Python 3.14**, application **tkinter**.
- Code source dans `__src__/`, tests dans `__tests__/`.
- Framework : **pytest** + **pytest-cov** (configuration déjà dans le `pyproject.toml`).
...
```
pytest -v --cov=__src__ --cov-report=term-missing
```
...
2. **Lancer la couverture initiale** : exécute `pytest` avec la commande ci-dessus et lis le rapport `term-missing` pour identifier précisément les lignes non couvertes (colonne *Missing*).
3. **Écrire/compléter les tests** dans `__tests__/`, en priorisant les modules les moins couverts et la logique métier (pas le boilerplate tkinter).
4. **Relancer** pytest après chaque ajout, lire à nouveau le `term-missing`, et **itérer** jusqu'à ≥ 90 % global.
...
- **Ne crée pas de vraie fenêtre** : mocke `tkinter.Tk`, les widgets, les `StringVar`/`IntVar` et les callbacks. Vérifie que les bons appels sont déclenchés (ex. `mock_label.config.assert_called_with(...)`) plutôt que de rendre une interface.
...
- Si 90 % est inatteignable sans tester du code mort ou du pur boilerplate tkinter, explique pourquoi et propose des marqueurs `# pragma: no cover` ciblés et justifiés.
