...
from ..core.models import DirtyRegion, DragState  # noqa: F401
...
class DragDropList(tk.Frame, Generic[T]):
    ...
    def _draw_insert_line(self, pos: int) -> None:
        ...
        """
        ds = self._drag_state
        gap_h = self._calc._pad + (
            self._calc._gap_expand if ds is not None and ds.expand_gap == pos else 0
        )
    ...
    def _draw_floating_and_line(
        ...
        if ds is None or ds.insert_pos is None:
            return
        gap_h = self._calc._pad + (
            self._calc._gap_expand if ds.expand_gap == ds.insert_pos else 0
        )
    ...
    def redraw_visible(self, force: bool = False) -> None:
        ...
        if (
            not force
            and self._last_visible_range == current
            and self._last_buttons_range == current_buttons
        ):
            return
    ...
    def _on_drag(self, event: tk.Event[tk.Canvas]) -> None:
        ...
        effective_pos = (
            None if insert_pos in (ds.drag_idx, ds.drag_idx + 1) else insert_pos
        )
