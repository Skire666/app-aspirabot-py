...
class StepsListPresenter:
    ...
    def __init__(
        ...
        # All Playwright calls are routed through _debug_queue to the single
        # browser worker thread (sync_playwright is bound to its creation thread).
    ...
    def _bind_view_events(self) -> None:
        ...
        self._view.on_debug_page = self._on_debug_page
    ...
    def _on_debug_page(self, url: str) -> None:
        """Opens a debug browser session for the given URL.

        Closes any prior session, creates a fresh queue and a single
        persistent browser worker thread that owns all Playwright calls.

        Args:
            url: The URL to open in the debug browser.
        """
        self._close_debug_session()
        # Fresh queue for this session — old worker reads None from the old queue.
    ...
    def _close_debug_session(self) -> None:
        """Stops the browser worker and destroys the debug window."""
        if self._debug_window is not None:
            with contextlib.suppress(Exception):
                self._debug_window.destroy()
            self._debug_window = None
        # Sentinel None stops the worker loop; browser close is in the finally block.
    ...
    def _browser_worker(self, url: str) -> None:
        ...
        The browser is closed in the finally block regardless of any error.
    ...
    def _task_refresh(self, page: Page) -> None:
        ...
        Runs inside the browser worker thread via the task queue.
    ...
    def _format_text_results(selector: str, result: dict[str, object]) -> str:
        ...
        lines = [
            ...
            "",
            f"textContent :\n{result.get('textContent', '')}",
            "",
            f"innerText   :\n{result.get('innerText', '')}",
            "",
            f"outerText   :\n{result.get('outerText', '')}",
        ]
        if count > 1:
