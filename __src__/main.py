...
def _build_and_wire_components(  # noqa: PLR0914
    ...
    main_view.set_on_show(TitleModuleEnum.E_DISCOVER, disc_pr.ensure_data_loaded)
    views: list[tk.Widget] = [
        log_view, profiles_view, cfg_view, scen_view, edit_view,
        exec_view, scrap_view, dbg_view, disc_view,
    ]
    presenters: list[object] = [
        log_pr, cfg_pr, prof_pr, scen_pre, edit_pr, steps_pr,
        exec_pre, scrap_pre, dbg_p, disc_pr,
    ]
...
def _init_discover_component(
    ...
    service = DiscoverService(repo, profiles_service)
    vm = DiscoverViewModel(master=main_view.content_area)
    view = DiscoverView(main_view.content_area, vm=vm)
    presenter = DiscoverPresenter(vm=vm, service=service)
