...
Freezes the contract of UrlsManualListService:
  ...
  - is_ready_to_consum_urls() correctly reports availability
...
class TestCountUrls:
    ...
    def test_count_does_not_change_after_advance(self) -> None:
        svc = _make_service(["http://a.com", "http://b.com"])
        svc.load_next_url()
...
class TestPreviewAllUrls:
    ...
    def test_preview_does_not_advance_cursor(self) -> None:
        ...
        assert svc.read_current_url() == "http://a.com", "preview_all_urls must not advance the cursor"


# ---------------------------------------------------------------------------
# is_ready_to_consum_urls / load_next_url / reset
# ---------------------------------------------------------------------------


class TestIsReadyAndAdvance:
    def test_is_ready_true_when_urls_available(self) -> None:
    ...
    def test_read_current_url_none_after_exhaustion(self) -> None:
        svc = _make_service(["http://a.com"])
        svc.load_next_url()
        assert svc.read_current_url() is None

    def test_load_next_url_raises_when_not_ready(self) -> None:
        svc = UrlsManualListService()
        with pytest.raises(UrlSourceExhaustedError):
            svc.load_next_url()

    def test_read_current_url_returns_urls_in_order(self) -> None:
        svc = _make_service(["http://first.com", "http://second.com"])
        assert svc.read_current_url() == "http://first.com"
        svc.load_next_url()
        assert svc.read_current_url() == "http://second.com"

    def test_reset_rewinds_cursor(self) -> None:
        svc = _make_service(["http://a.com", "http://b.com"])
        svc.load_next_url()
        svc.reset()
        assert svc.read_current_url() == "http://a.com", "After reset(), cursor must rewind to first URL"

    def test_read_current_url_before_and_after_advance(self) -> None:
        svc = _make_service(["http://a.com", "http://b.com"])
        assert svc.read_current_url() == "http://a.com"
        svc.load_next_url()
        assert svc.read_current_url() == "http://b.com"

    def test_read_current_url_none_when_exhausted(self) -> None:
        svc = _make_service(["http://a.com"])
        svc.load_next_url()
        assert svc.read_current_url() is None
